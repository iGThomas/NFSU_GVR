# ============================================================
# Install-NFSU-GVR-SQLite.ps1  (Release V2 - SQLite edition)
#
# Standalone installer for Need For Speed Underground: GlobalVR.
# Prompts for the OEM Disc 1 / Disc 2, extracts and installs the game,
# then wires it to run on an embedded SQLite database instead of
# MSDE / SQL Server 2000.
#
#   - NO C\Program Files\Microsoft SQL Server (MSDE)
#   - NO C\Program Files\SQLXML 3.0
#   - NO osql import / sa rotation / reboot
#   The game's PLUSDE.dll is patched to talk to a bundled SQLite
#   provider (GvrSqlite.dll -> sqlite3.dll) against a seeded game.db.
#
# This package does not include the game payload itself; you provide
# the original OEM Disc 1 and Disc 2 (or -ExpandedPayloadRoot).
# ============================================================
...
$AioVersion = "2026-07-17-sqlite-v2"
...
$DbPath = Join-Path $TargetDrive "GvrPlus\game.db"
...
function Invoke-CoreInstaller([switch]$SkipGamePayloadCopy) {
    ...
    # SQLite edition: ALWAYS skip SQL Server / MSDE and the Disc 2 osql DB import.
    # DirectX is handled by this installer (Install-DirectX9), so skip the core's DX step.
    $args += "-SkipSql"
    $args += "-SkipDirectXCheck"
...
function Install-DirectX9 {
    if ($SkipDirectXCheck) { Write-Log "DirectX step skipped (-SkipDirectXCheck)"; return }
    $dx = @((Join-Path $ReleaseRoot "Dependencies\DirectX9\dxsetup.exe"),
            (Join-Path $ReleaseRoot "Dependencies\DirectX9\DXSETUP.exe")) |
          Where-Object { Test-Path $_ } | Select-Object -First 1
    if (!$dx) { Write-Log "No bundled DirectX9\dxsetup.exe; ensure Direct3D 9 is present"; return }
    Write-Log "Installing DirectX 9 runtime (silent)"
    if (!$DryRun) { Start-Process -FilePath $dx -ArgumentList "/silent" -Wait }
}

function Deploy-SqliteBackend {
    $sqliteDir = Join-Path $ReleaseRoot "SQLite"
    foreach ($f in @("PLUSDE.dll","GvrSqlite.dll","GvrSqlite.cs","sqlite3.dll","game.db")) {
        if (!(Test-Path (Join-Path $sqliteDir $f))) { Fail "Missing SQLite payload file: $f (in $sqliteDir)" }
    }
    Write-Log "Deploying SQLite backend (this replaces MSDE / SQL Server entirely)"

    # 1) replace every PLUSDE.dll under the target drive + system dirs with the patched engine
    $roots = @(($TargetDrive + "\"), (Join-Path $env:WINDIR "System32"), (Join-Path $env:WINDIR "SysWOW64"))
    $found = @()
    foreach ($rt in $roots) {
        if (Test-Path $rt) { $found += @(Get-ChildItem -Path $rt -Filter PLUSDE.dll -Recurse -ErrorAction SilentlyContinue | Where-Object { !$_.PSIsContainer }) }
    }
    $found = $found | Sort-Object FullName -Unique
    foreach ($t in $found) {
        if ($t.DirectoryName -ieq $sqliteDir) { continue }
        Write-Log "  PLUSDE.dll -> $($t.FullName)"
        if (!$DryRun) {
            $bak = "$($t.FullName).sqlbak"
            if (!(Test-Path $bak)) { Copy-Item $t.FullName $bak -Force }
            Copy-Item (Join-Path $sqliteDir "PLUSDE.dll") $t.FullName -Force
        }
    }
    Write-Log ("  PLUSDE.dll copies patched: {0}" -f $found.Count)

    # 2) provider (built on target with the machine's 1.1 csc; prebuilt 1.1 fallback) + sqlite3.dll
    #    beside every EXE that hosts the DB engine
    $csc = Join-Path $env:WINDIR "Microsoft.NET\Framework\v1.1.4322\csc.exe"
    $exeDirs = @{}
    foreach ($exe in @("UndergroundGVR.exe","UniverShell2.exe","db_propagate.exe","GVRDongleMonitor.exe")) {
        Get-ChildItem -Path ($TargetDrive + "\") -Filter $exe -Recurse -ErrorAction SilentlyContinue | Where-Object { !$_.PSIsContainer } | ForEach-Object { $exeDirs[$_.DirectoryName] = $true }
    }
    if ($exeDirs.Count -eq 0) { Warn "No game/shell EXEs found under $TargetDrive; SQLite provider not placed." }
    foreach ($d in $exeDirs.Keys) {
        Write-Log "  provider + sqlite3 -> $d"
        if ($DryRun) { continue }
        Copy-Item (Join-Path $sqliteDir "sqlite3.dll") (Join-Path $d "sqlite3.dll") -Force
        $built = $false
        if (Test-Path $csc) {
            $cs = Join-Path $d "GvrSqlite.cs"
            Copy-Item (Join-Path $sqliteDir "GvrSqlite.cs") $cs -Force
            # native .NET 1.1 build - the 2.0-compiled+retargeted binary loads as BadImageFormat on CLR 1.1
            & $csc /nologo /target:library /out:(Join-Path $d "GvrSqlite.dll") /r:System.dll /r:System.Data.dll $cs 2>&1 | Out-Null
            if (Test-Path (Join-Path $d "GvrSqlite.dll")) { $built = $true }
            Remove-Item $cs -Force -ErrorAction SilentlyContinue
        }
        if (!$built) {
            Warn "  .NET 1.1 csc unavailable in $d; using bundled prebuilt GvrSqlite.dll"
            Copy-Item (Join-Path $sqliteDir "GvrSqlite.dll") (Join-Path $d "GvrSqlite.dll") -Force
        }
    }

    # 3) install the shared seeded db + machine env var (shared by cabinet + shell)
    Write-Log "  installing seeded db -> $DbPath"
    if (!$DryRun) {
        New-Dir (Split-Path -Parent $DbPath)
        if (!(Test-Path $DbPath)) { Copy-Item (Join-Path $sqliteDir "game.db") $DbPath -Force }
        else { Write-Log "  db already exists, keeping it (delete to reseed)" }
        [Environment]::SetEnvironmentVariable("GVRSQLITE_DB", $DbPath, "Machine")
    }
    Write-Log "  GVRSQLITE_DB = $DbPath"
    Write-Log "SQLite backend deployed. No SQL Server / MSDE / SQLXML installed."
}
...
Install-DirectX9
Deploy-SqliteBackend

Write-Log "SQLite-edition install complete. The game runs on SQLite - no SQL Server / MSDE / SQLXML."
Clear-ResumeMarker
if ($KeepWorkDir) {
